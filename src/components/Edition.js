...
export const Edition = (props) => {
    ...
    const token_type = props.pathArgs[1]
    ...
    return <section className="edition">
        <header>
            ...
            <img src={Close} onClick={() => history.back()} />
        </header>
        <div>
            ...
            {
                editions.map(({ token_id, edition_id }) => <div key={edition_id} onClick={() => history.push('/sale/' + token_id)}>
                    <div>#{edition_id}</div>
